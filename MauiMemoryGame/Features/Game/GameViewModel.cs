...
public class GameViewModel : BaseViewModel, IQueryAttributable
{
    ...
    public GameViewModel(INavigationService navigationService, ILogService logService, IDialogService dialogService) : base(navigationService, logService)
	{
    ...
    [Reactive] public int CardPairsFound { get; private set; }
    ...
    public TimeSpan TotalTime => SelectedLevel switch
    {
        ...
        Level.Medium => TimeSpan.FromSeconds(10), //TimeSpan.FromMinutes(4),
    ...
    protected override void CreateCommands()
    {
        ...
        EqualsCardsCommand = ReactiveCommand.CreateFromTask<Tuple<Card, Card>, bool>(EqualsCardsAsync);
    }
    ...
    private async Task<bool> EqualsCardsAsync(Tuple<Card, Card> cards)
    {
        AttempsNumber++;
        if (cards.Item1.ImagePath != cards.Item2.ImagePath)
            return false;

        cards.Item1.Found = true;
        cards.Item2.Found = true;
        CardPairsFound++;
